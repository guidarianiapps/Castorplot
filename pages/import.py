...
with colunas_import[0]:
    ...
    if uploaded_file == []:
        ...
        st.stop()
    ignor_cabecalho = st.number_input("Linha do cabeçalho?", value=0, min_value=0,help="Linha do cabeçalho, define a linha que será utilizada como cabeçalho, automaticamente se a primeira linha tiver somente números os nomes serão trocados automaticamente. Futuramente poderá ser trocado o nome de cada linha diretamente no site.")

    delimitador = st.text_input("Qual é o delimitador de coluna?", value="\\t",help="""Delimitador de coluna: É o delimitador  de coluna, por padrão utiliza \\t, pois é como se interpreta o "tab", outros parâmetros como "," e ";" é somente escrever, qualquer dúvida concute a [documentação](https://pandas.pydata.org/docs/reference/api/pandas.read_csv.html#:~:text=ou%20StringIO.-,sep,-str%2C%20padr%C3%A3o%20%27%2C%27).""")

    separador = st.text_input("Qual é o separador decimal?", value=".", help= """                 
